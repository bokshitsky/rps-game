// Based on "QRCode for JavaScript" by Kazuhiko Arase (MIT), adapted to local TypeScript module use.

const pad0 = 0xec;
const pad1 = 0x11;

const qrMode8BitByte = 1 << 2;
const errorCorrectLevelL = 1;
...
class Polynomial {
  ...
  multiply(other: Polynomial): Polynomial {
    const num = new Array<number>(this.getLength() + other.getLength() - 1).fill(0);
...
function getRsBlocks(typeNumber: number, errorCorrectLevel: number): RsBlock[] {
  if (errorCorrectLevel !== errorCorrectLevelL) {
    throw new Error("Only level L is supported");
  }
  const rsBlock = rsBlockTable[(typeNumber - 1) * 4];
...
function createData(typeNumber: number, errorCorrectLevel: number, dataList: Qr8BitByte[]): number[] {
...
function getLostPoint(qrCode: QrCode): number {
...
export function createQrSvgDataUrl(text: string): string {
  ...
  const qr = new QrCode(0, errorCorrectLevelL);
  qr.addData(text);
  qr.make();
